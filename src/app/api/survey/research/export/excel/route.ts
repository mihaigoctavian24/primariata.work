  ...
  try {
    ...
    if (insights && insights.length > 0) {
      ...
      insights.forEach((insight) => {
        insightsSheet.addRow([
          insight.survey_type === "citizen" ? "Cetățean" : "Funcționar",
      ...
      themesSheet.addRow(["Tip Sondaj", "Temă", "Scor", "Mențiuni", "Sentiment"]);
        ...
        if (themes && Array.isArray(themes)) {
          themes.forEach((themeObj) => {
            if (
              ...
              themesSheet.addRow([
                insight.survey_type === "citizen" ? "Cetățean" : "Funcționar",
                (themeObj as { theme: string }).theme,
                (themeObj as { score: number }).score?.toFixed(2) || "N/A",
                (themeObj as { mentions: number }).mentions || 0,
                (themeObj as { sentiment?: number }).sentiment?.toFixed(2) || "N/A",
              ]);
            }
          });
        }
