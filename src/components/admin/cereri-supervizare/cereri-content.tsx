...
function CereriContent({ cereri, functionari }: CereriContentProps): React.ReactElement {
  ...
  // Optimistic status change handler (Kanban click-to-move)
  const handleStatusChange = useCallback(
    async (cerereId: string, newDbStatus: string): Promise<void> => {
      ...
      if (result.error) {
        ...
        setLocalCereri((prev) =>
          prev.map((c) => (c.id === cerereId ? { ...c, status: c.status } : c))
        );
        ...
        >
          ...
          {activeTab === "table" && (
            <CereriTableTab
              ...
              onCerereAction={() => {
                // Table actions handled inline; no optimistic state change needed here
              }}
            />
          )}
