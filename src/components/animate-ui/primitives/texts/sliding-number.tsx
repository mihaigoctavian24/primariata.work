...
function SlidingNumberRoller({
  ...
  // Add 20 to make 2 full cycles (0→9→0→9→target)
  const animatedValue = useSpring(startNumber, transition);

  React.useEffect(() => {
    const timeoutId = setTimeout(() => {
      // Animate through 2 full cycles before reaching target
      animatedValue.set(targetNumber + 20);
    }, delay);
...
function SlidingNumber({
  ...
  React.useEffect(() => {
    ...
    // Else branch: set effective number when not animating
  ...
  const splitNumber = numberStr.split(".");
  const newIntStrRaw = splitNumber[0] ?? "0";
  const newDecStrRaw = splitNumber[1] ?? "";
